...
class SideLayoutSolver(CalloutLayoutSolver):
        ...
        for unusedLength, pointId, item, baseline in sorted(pending):
            ...
            if self._overlap(baseline, placed) > EPSILON:
                ...
                for row in sorted(rows):
                    option = candidate(item, baseline['lane'], row)
    ...
    def _overlap(self, candidate, placed):
        ...
        return sum(rectangleIntersectionArea(padded, entry['rect'])
                   for entry in placed)

    def _rank(self, candidate, item, placed):
        ...
        for existing in placed:
            ...
            if existing['lane'] == candidate['lane']:
                ...
                inversions += int(pointDelta * (centerY - otherCenter)
                                  < -EPSILON)
        # A crossing costs about one row, never an unlimited detour.
        rowHeight = float(item['height']) + LABEL_GAP
        cost = displacement + rowHeight * (crossings + inversions)
        return (self._overlap(candidate, placed), cost, displacement,
                _polylineLength(candidate['leader']), centerY)
