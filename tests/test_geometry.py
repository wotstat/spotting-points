...
class GeometryTests(unittest.TestCase):
    ...
    def test_turret_circle_closes_around_the_rotation_axis(self):
        from wotstat_spotting_points import geometry

        self.assertTrue(hasattr(geometry, 'buildTurretCircle'))
        points = geometry.buildTurretCircle((1, 2, 3), (1, 3, 5))

        self.assertGreaterEqual(len(points), 25)
        self.assertLessEqual(len(points), 37)
        self.assertEqual(points[0], (1, 3, 5))
    ...
    def test_highlight_groups_split_faces_from_active_guides(self):
        ...
        self.assertEqual(groups['front'].faded, [])
        self.assertEqual(len(groups['front'].bright), 3)
        self.assertEqual(groups['front'].bright[0].points,
                         [(-2, 1, 6), (-2, 3, 6),
                          (2, 3, 6), (2, 1, 6), (-2, 1, 6)])
        self.assertEqual(groups['front'].bright[1].points,
                         [(-2, 3, 6), (2, 1, 6)])
        self.assertEqual(groups['front'].bright[2].points,
                         [(-2, 1, 6), (2, 3, 6)])
        self.assertEqual(groups['rear'].faded, [])
        self.assertEqual(len(groups['rear'].bright), 3)
        self.assertEqual(len(groups['left'].faded), 3)
        self.assertEqual(groups['left'].faded[1].points,
                         [(-2, 1, -4), (-2, 3, 6)])
        self.assertEqual(groups['left'].faded[2].points,
                         [(-2, 3, -4), (-2, 1, 6)])
        self.assertEqual(groups['left'].bright[0], lines[7])
        self.assertEqual(groups['left'].bright[1].points,
                         [(0, 3.5, 1.8), (-2, 3.5, 1.8)])
        self.assertEqual(len(groups['right'].faded), 3)
        self.assertEqual(groups['right'].faded[1].points,
                         [(2, 1, 6), (2, 3, -4)])
        self.assertEqual(groups['right'].faded[2].points,
                         [(2, 3, 6), (2, 1, -4)])
        self.assertEqual(groups['right'].bright[0].points,
                         [(0, 3.5, 1.8), (2, 3.5, 1.8)])
        self.assertEqual(groups['right'].bright[1], lines[9])
        self.assertEqual(groups['top'].faded, [lines[0]])
        self.assertEqual(groups['top'].bright, [lines[10]])
        self.assertEqual(groups['gunStatic'].faded, [])
        self.assertEqual(groups['gunStatic'].bright, [lines[8]])

    def test_moving_gun_highlight_uses_circle_and_arc_without_axis(self):
        from wotstat_spotting_points.geometry import (
        ...
        arc = LineGeometry([(0, 3.5, 1.8), (1, 3.5, 1.0)],
                           0x959595, 0x646464)
        circle = LineGeometry([(0, 3.5, 1.8), (0, 3.5, -1.8),
                               (0, 3.5, 1.8)], 0x959595, None)

        groups = buildHighlightGroups(lines, arc, circle)

        self.assertEqual(groups['gunMoving'].faded, [circle])
        self.assertEqual(groups['gunMoving'].bright, [arc])
        self.assertNotIn(lines[8], groups['gunMoving'].faded)
        self.assertNotIn(lines[8], groups['gunMoving'].bright)
