...
**Architecture:** AS3 samples current projected anchors, marker positions, and label sizes, then calls Python once in the same frame. A game-independent Python solver builds convex obstacles, candidates, exact leader polylines, lexicographic costs, and a bounded beam-search plan. AS3 applies a changed revision atomically and only draws the returned result.
...
- [ ] **Step 1: Add one focused failing geometry/candidate test**
...
- [ ] **Step 2: Implement the minimal pure geometry and candidate model**
...
- [ ] **Step 3: Add focused failing solver tests**
...
- [ ] **Step 4: Implement local scoring and beam search**

Precompute candidate-local overflow, forbidden-overlap, leader length, and lane-switch components. During expansion add label overlap, leader crossings, and bidirectional foreign-point clearance. Sort partial plans by the lexicographic tuple and stable candidate signature, retaining `BEAM_WIDTH = 64`.

On a changed valid signature, update all remembered lanes together and return the full result with a new revision. On an unchanged quarter-pixel signature, return only that revision. Remove lane memory for absent ids.

- [ ] **Step 5: Run the focused solver tests and commit the slice**
...
- [ ] **Step 1: Add one failing bridge/lifecycle test**
...
- [ ] **Step 2: Implement bridge conversion and timing**
...
- [ ] **Step 3: Implement fail-once behavior**
...
- [ ] **Step 4: Run marker-view and solver tests and commit**
...
- [ ] **Step 1: Replace the AS3 optimizer with payload sampling**
...
- [ ] **Step 2: Apply only changed complete revisions**
...
- [ ] **Step 3: Draw returned leaders and geometry verbatim**
...
- [ ] **Step 4: Remove the obsolete AS3 geometry class and compile**

```powershell
./build.ps1 -Version 0.3.0 -Python C:/Python27/python.exe
```

- [ ] **Step 5: Commit the AS3 migration**
...
- [ ] **Step 2: Run full automated verification and inspect packages**
...
- [ ] **Step 3: Install only the rebuilt RU package**
...
- [ ] **Step 4: Validate behavior through WotStat REPL**
...
- [ ] **Step 5: Fix only concrete runtime defects, rerun proportional checks, and commit docs/final fixes**
...
- [ ] **Step 6: Final cleanliness check and leave the RU client running**
